...
OS::OS()
{
    ...
    while (progs >> prog) {
		...
		string stackFile = prog_name + ".st"; 
		stackcode.open(stackFile.c_str(), ios::out);
		stackcode.close();
		
        PCB * job = new PCB(prog_name, base, limit-base);
        
        int pts = (job->limit)/8;	// get the size of the page table for each process.        
        //if ((job->limit)%8 != 0)
        	//pts += 1;
        
        job->pagetable = new PT(pts);	// creats every PCBs pagetable.
        
        job->pagetable->fill_frames(fcount);	// sets the frame # for everything in physical memeory.
        fcount = fcount + pts + 1;
        
        cout << "Frame # in process: " << prog_name << endl;
        
        for(int i = 0; i < pts + 1; i++){
        	cout << job->pagetable->page_table[i].frame << endl;
        }
...
void OS::contextSwitch() 
{
    ...
    if((vm.sr >> 9) & 1 == 1){	// check PF bit
    	running->io_completion = vm.clock + 27;	// adds 27 clock tick IO operation.
        running->io_time_begin = vm.clock;	
    	vm.sr = vm.sr & 64511;
    	waitQ.push(running);
    }
